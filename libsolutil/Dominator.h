...
private:
	std::vector<std::optional<DfsIndex>> findDominators(V const& _entry)
	{
		...
		auto dfs = [&](V const& _v, auto _dfs) -> void {
			...
			ForEachSuccessor{}(_v, [&](V const& _successor) {
				if (visited.count(_successor.id) == 0)
				{
					...
					predecessors.push_back({m_dfsIndexByVertexId[_v.id]});
					_dfs(_successor, _dfs);
				}
				else
				{
					solAssert(m_dfsIndexByVertexId[_successor.id] < predecessors.size());
					predecessors[m_dfsIndexByVertexId[_successor.id]].insert(m_dfsIndexByVertexId[_v.id]);
				}
			});
		};
		...
		solAssert(predecessors.size() == numVertices);
